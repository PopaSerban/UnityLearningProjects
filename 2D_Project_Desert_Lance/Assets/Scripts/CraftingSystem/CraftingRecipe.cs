...
public class CraftingRecipe : ScriptableObject
{
	...
	{
		var duplicates = Materials.GroupBy( x => x.Item.ID)
		...
		foreach (ItemAmount itemAmount in Materials)
		{
			...
			{	
				idHolder = duplicate.Key;
				if(itemAmount.Item.ID == idHolder)
				{
					duplicateAmount = duplicate.Value;
				}
			}
			if (itemContainer.ItemCount(itemAmount.Item.ID) < duplicateAmount)
			{
	...
	private void AddResults(IItemContainer itemContainer)
	{
		foreach (ItemAmount itemAmount in Results)
		{
            Debug.Log("ItemAmount : "+itemAmount.Amount);
			for (int i = 0; i < itemAmount.Amount; i++)
			{
